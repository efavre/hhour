class PicturesController < ApplicationController
	...
	def index
		if params[:picture_thread_id] && PictureThread.find(params[:picture_thread_id])
	...
	def create
		if params[:picture_thread_id] && PictureThread.find(params[:picture_thread_id])
			...
			if params[:picture][:author]
				...
				picture = @picture_thread.pictures.create(file_key: params[:picture][:file_key], author: author)
				if picture.valid?
					render action: :index
				else
